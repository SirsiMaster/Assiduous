...
class githubdataprocessor {
    ...
    extractSessionMetadata(commitMessage) {
        ...
        const typeMatch = commitMessage.match(/^(feat|fix|docs|style|refactor|test|chore|perf|ci|build)(\([^)]+\))?:/);
    ...
    calculateTimeSpan(commits) {
        ...
        const last = new Date(commits[commits.length - 1].timestamp || commits[commits.length - 1].commit?.committer?.date);
