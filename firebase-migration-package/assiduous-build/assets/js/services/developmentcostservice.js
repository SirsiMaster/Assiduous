...
class developmentcostservice {
  ...
  calculateTotalCosts() {
    ...
    totals.thisWeek.cloudCost = 0.5; // Minimal usage so far
    ...
    Object.keys(totals).forEach(period => {
      totals[period].total =
        totals[period].laborCost + totals[period].toolsCost + totals[period].cloudCost;
    });
  ...
  calculateTrends() {
    ...
    this.workSessions.forEach(session => {
      ...
      dailyData[dayKey].total =
        dailyData[dayKey].laborCost + dailyData[dayKey].toolsCost + dailyData[dayKey].cloudCost;
    ...
    Object.keys(weeklyData).forEach(week => {
      ...
      weeklyData[week].total =
        weeklyData[week].laborCost + weeklyData[week].toolsCost + weeklyData[week].cloudCost;
    });
  ...
  exportForPDF() {
    ...
    return {
      ...
      sessions: this.workSessions.map(s => ({
        ...
        totalCost:
          (s.cost || s.hoursWorked * this.hourlyRate) +
          this.calculateMonthlyToolsCost() / 30 +
          (s.cloudCost || 0),
      })),
      ...
      totals: {
        ...
        laborCost: this.workSessions.reduce(
          (sum, s) => sum + (s.cost || s.hoursWorked * this.hourlyRate || 0),
          0
        ),
